...
Для выполнения операций сборки выполняемых программ из исходных кодов на C/C++ необходимо подготовить рабочее окружение Build-системы.
Для этого исполюзуют подготовленные скрипты, которые расположены в папке Visual Studio
```
...
# Добавить папку сборки проекта в перечень доступных путей ОС.
set path = %~dp;%path%
```
